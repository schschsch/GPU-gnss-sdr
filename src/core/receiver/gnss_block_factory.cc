...
#if CUDA_GPU_ACCEL
            else if (implementation == "BEIDOU_B1I_DLL_PLL_Tracking_GPU")
                {
                    std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<BeidouB1iDllPllTrackingGPU>(configuration, role , in_streams,
                        out_streams);
                        block = std::move(block_);
                }
    ...
    if (implementation == "GPS_L1_CA_DLL_PLL_Tracking")
    ...
    else if (implementation == "BEIDOU_B3I_DLL_PLL_Tracking")
        {
            ...
            block = std::move(block_);
                ...
                {
                    std::unique_ptr<TrackingInterface> block_ = std::make_unique<BeidouB1iDllPllTrackingGPU>(configuration, role , in_streams,
                        out_streams);
                        block = std::move(block_);
                }
    else if (implementation == "GPS_L1_CA_DLL_PLL_Tracking_GPU")
