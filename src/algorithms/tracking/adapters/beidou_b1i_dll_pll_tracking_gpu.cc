/*!
 * \file beidou_b1i_dll_pll_tracking_gpu.cc
...
#include "beidou_b1i_dll_pll_tracking_gpu.h"
...
BeidouB1iDllPllTrackingGPU::BeidouB1iDllPllTrackingGPU(
      ...
      out_streams_(out_streams)
{
    ...
    if (trk_params.item_type == "gr_complex")
        {
            tracking_sptr_ = dll_pll_veml_make_tracking_gpu(trk_params);
...
void BeidouB1iDllPllTrackingGPU::start_tracking()
{
    tracking_sptr_->start_tracking();
}


void BeidouB1iDllPllTrackingGPU::stop_tracking()
{
 ...
 */
void BeidouB1iDllPllTrackingGPU::set_channel(unsigned int channel)
{
    channel_ = channel;
...
void BeidouB1iDllPllTrackingGPU::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
    tracking_sptr_->set_gnss_synchro(p_gnss_synchro);
}


void BeidouB1iDllPllTrackingGPU::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void BeidouB1iDllPllTrackingGPU::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
...
gr::basic_block_sptr BeidouB1iDllPllTrackingGPU::get_left_block()
{
    return tracking_sptr_;
}


gr::basic_block_sptr BeidouB1iDllPllTrackingGPU::get_right_block()
{
    return tracking_sptr_;
}
