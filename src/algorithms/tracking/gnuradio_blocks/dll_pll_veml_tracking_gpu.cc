/*!
 * \file dll_pll_veml_tracking_gpu.cc
...
#include "dll_pll_veml_tracking_gpu.h"
...
dll_pll_veml_tracking_gpu_sptr dll_pll_veml_make_tracking_gpu(const Dll_Pll_Conf &conf_)
{
    return dll_pll_veml_tracking_gpu_sptr(new dll_pll_veml_tracking_gpu(conf_));
}


dll_pll_veml_tracking_gpu::dll_pll_veml_tracking_gpu(const Dll_Pll_Conf &conf_)
    : gr::block("dll_pll_veml_tracking_gpu", gr::io_signature::make(1, 1, sizeof(gr_complex)),
      ...
      d_Flag_PLL_180_deg_phase_locked(false)
{
    ...
    this->set_msg_handler(
        ...
        boost::bind(&dll_pll_veml_tracking_gpu::msg_handler_telemetry_to_trk, this, boost::placeholders::_1));
#else
        boost::bind(&dll_pll_veml_tracking_gpu::msg_handler_telemetry_to_trk, this, _1));
#endif
#endif
    ...
    cudaHostAlloc((void**)(&in_gpu), 2 * d_trk_parameters.vector_length * sizeof(gr_complex), cudaHostAllocMapped || cudaHostAllocWriteCombined);
    ...
    if (d_trk_parameters.track_pilot)
        {
            ...
            d_correlator_data_gpu.init(static_cast<int>(2 * d_trk_parameters.vector_length),1);
            d_correlator_data_gpu.set_high_dynamics_resampler(d_trk_parameters.high_dyn);
            d_data_code.resize(2 * d_code_length_chips, 0.0);
        }

    // --- Initializations ---
    d_Prompt_circular_buffer.set_capacity(d_secondary_code_length);
    d_multicorrelator_gpu.set_high_dynamics_resampler(d_trk_parameters.high_dyn);

    // CN0 estimation and lock detector buffers
    d_Prompt_buffer = volk_gnsssdr::vector<gr_complex>(d_trk_parameters.cn0_samples);
    cudaHostAlloc((void**)(&d_Prompt_Data),sizeof(gr_complex),cudaHostAllocMapped || cudaHostAllocWriteCombined);
...
void dll_pll_veml_tracking_gpu::forecast(int noutput_items,
...
void dll_pll_veml_tracking_gpu::msg_handler_telemetry_to_trk(const pmt::pmt_t &msg)
{
    try
        {
...
void dll_pll_veml_tracking_gpu::start_tracking()
{
            ...
            if (d_trk_parameters.track_pilot)
                {
                    ...
                    d_correlator_data_gpu.set_local_code_and_taps(d_code_length_chips, d_data_code.data(), d_prompt_data_shift);
                }
            else
                {
            ...
            if (d_trk_parameters.track_pilot)
                {
                    ...
                    d_Prompt_Data[0] = gr_complex(0.0, 0.0);
                    d_correlator_data_gpu.set_local_code_and_taps(d_code_samples_per_chip * d_code_length_chips, d_data_code.data(), d_prompt_data_shift);
                }
            else
                {
    ...
    d_multicorrelator_gpu.init_cuda_integrated_resampler(static_cast<int>(2 * d_trk_parameters.vector_length), d_code_samples_per_chip * d_code_length_chips, d_n_correlator_taps);
    d_multicorrelator_gpu.set_local_code_and_taps(d_code_samples_per_chip * d_code_length_chips,d_tracking_code.data(), d_local_code_shift_chips.data());
...
dll_pll_veml_tracking_gpu::~dll_pll_veml_tracking_gpu()
{
    ...
    try
        {
            if (d_trk_parameters.track_pilot)
                {
                    d_correlator_data_gpu.free_cuda();
                }
            d_multicorrelator_gpu.free_cuda();
        }
    catch (const std::exception &ex)
        {
            LOG(WARNING) << "Exception in Tracking block destructor: " << ex.what();
...
bool dll_pll_veml_tracking_gpu::acquire_secondary()
{
...
bool dll_pll_veml_tracking_gpu::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
...
void dll_pll_veml_tracking_gpu::do_correlation_step(const gr_complex *input_samples)
{
    // ################# CARRIER WIPEOFF AND CORRELATORS ##############################
    // perform carrier wipe-off and compute Early, Prompt and Late correlation
    d_multicorrelator_gpu.set_input_output_vectors(d_correlator_outs, input_samples);
    d_multicorrelator_gpu.Carrier_wipeoff_multicorrelator_resampler_cuda(
    ...
    if (d_trk_parameters.track_pilot)
        {
            d_correlator_data_gpu.set_input_output_vectors(d_Prompt_Data, input_samples);
            d_correlator_data_gpu.Carrier_wipeoff_multicorrelator_resampler_cuda(
                d_rem_carr_phase_rad,
                static_cast<float>(d_carrier_phase_step_rad), static_cast<float>(d_carrier_phase_rate_step_rad),
                static_cast<float>(d_rem_code_phase_chips) * static_cast<float>(d_code_samples_per_chip),
                static_cast<float>(d_code_phase_step_chips) * static_cast<float>(d_code_samples_per_chip),
...
void dll_pll_veml_tracking_gpu::run_dll_pll()
{
    // ################## PLL ##########################################################
    // PLL discriminator
    if (d_cloop)
...
void dll_pll_veml_tracking_gpu::check_carrier_phase_coherent_initialization()
{
...
void dll_pll_veml_tracking_gpu::clear_tracking_vars()
{
    std::fill_n(d_correlator_outs, d_n_correlator_taps, gr_complex(0.0, 0.0));
    if (d_trk_parameters.track_pilot)
        {
...
void dll_pll_veml_tracking_gpu::update_tracking_vars()
{
...
void dll_pll_veml_tracking_gpu::save_correlation_results()
{
    if (d_secondary)
        {
...
void dll_pll_veml_tracking_gpu::log_data()
{
    if (d_dump)
        {
...
int32_t dll_pll_veml_tracking_gpu::save_matfile() const
{
    // READ DUMP FILE
...
void dll_pll_veml_tracking_gpu::set_channel(uint32_t channel)
{
    gr::thread::scoped_lock l(d_setlock);
    d_channel = channel;
...
void dll_pll_veml_tracking_gpu::set_gnss_synchro(Gnss_Synchro *p_gnss_synchro)
{
    gr::thread::scoped_lock l(d_setlock);
...
void dll_pll_veml_tracking_gpu::stop_tracking()
{
    gr::thread::scoped_lock l(d_setlock);
    d_state = 0;
}
...
int64_t dll_pll_veml_tracking_gpu::uint64diff(uint64_t first, uint64_t second)
{
...
int dll_pll_veml_tracking_gpu::general_work(int noutput_items __attribute__((unused)), gr_vector_int &ninput_items,
    gr_vector_const_void_star &input_items, gr_vector_void_star &output_items)
{
    gr::thread::scoped_lock l(d_setlock);
    const auto *in = reinterpret_cast<const gr_complex *>(input_items[0]);
    std::copy(in,in+d_trk_parameters.vector_length,in_gpu);
